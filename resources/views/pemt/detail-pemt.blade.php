...
<head>
    ...
    <title>Detail Matriks PEMT - KILAT</title>
...
<body class="min-h-screen flex text-gray-800 overflow-x-hidden w-full">
    ...
    <div class="flex-1 pl-[72px] peer-hover:pl-64 flex flex-col min-h-screen transition-all duration-300 min-w-0">
        ...
        <main class="flex-1 p-5 md:p-8 animate-fade-in flex flex-col bg-gray-50/50 min-w-0 w-full">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-gray-800 tracking-tight">{{ request('satker', 'Ses. Ditjen PEMT') }}</h1>
            ...
            <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 bg-white p-2.5 rounded-2xl shadow-sm border border-gray-200">
                ...
                <div class="flex items-center gap-3 px-2">
                    <div class="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5"><span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Thn:</span><select class="bg-transparent text-gray-800 font-bold text-sm focus:outline-none outline-none"><option>2026</option><option>2025</option></select></div>
                    <div class="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5"><span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Bulan:</span><select class="bg-transparent text-gray-800 font-bold text-sm focus:outline-none outline-none"><option>Juli</option><option>Agustus</option></select></div>
                    <button class="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm ml-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>Excel</button>
                </div>
            </div>
                    ...
                    <table id="tabel-detail" class="w-full text-left border-collapse min-w-[1500px]">
                        ...
                        <tbody id="tbody-m1" class="tbody-minggu divide-y divide-gray-200 bg-white">
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3 text-center text-[10px] font-semibold text-gray-600 border-r border-gray-200" data-start="2026-07-01" data-end="2026-07-07">01 Jul - 07 Jul 2026</td>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm mb-8">
                ...
                <div class="flex items-center gap-2">
                    <!-- Tombol Hapus (Disembunyikan default, hanya muncul saat M5) -->
                    ...
                    <button id="btn-edit-data" onclick="toggleEditMode()" class="flex items-center gap-2 bg-gray-800 hover:bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md group">
                        ...
                        <span id="teks-btn-edit">Edit Data M1</span>
                    </button>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        function switchTab(minggu) { 
            ...
            document.querySelectorAll('.tab-minggu').forEach(b => b.className = 'tab-minggu px-5 py-2.5 text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-semibold text-sm rounded-xl whitespace-nowrap transition-all border border-transparent cursor-pointer'); 
            
            const activeTab = document.getElementById('tab-' + minggu);
            if(activeTab) activeTab.className = 'tab-minggu px-5 py-2.5 bg-blue-50/80 text-blue-700 font-extrabold text-sm rounded-xl whitespace-nowrap border border-blue-100 transition-all cursor-pointer'; 
            
            document.querySelectorAll('.tbody-minggu').forEach(t => t.classList.add('hidden')); 
            const activeTbody = document.getElementById('tbody-' + minggu);
            if(activeTbody) activeTbody.classList.remove('hidden'); 
            
            document.getElementById('label-total').innerText = 'TOTAL ' + minggu.toUpperCase(); 
            document.getElementById('teks-menampilkan').innerText = minggu.toUpperCase(); 
            
            // Logika Munculkan Tombol Hapus untuk M5
            const btnHapus = document.getElementById('btn-hapus-minggu');
            if (btnHapus) {
                if (minggu === 'm5') {
                    btnHapus.classList.remove('hidden');
                    btnHapus.classList.add('flex'); // Memunculkan tombol di M5
                } else {
                    btnHapus.classList.add('hidden');
                    btnHapus.classList.remove('flex'); // Menyembunyikan tombol di M1-M4
                }
            }
        ...
        function formatTanggalIndo(dStr) { if(!dStr) return ''; const m = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des']; const d = new Date(dStr); return isNaN(d) ? '' : `${String(d.getDate()).padStart(2,'0')} ${m[d.getMonth()]} ${d.getFullYear()}`; }
        ...
        function hapusBaris(e) { if(confirm('Hapus baris ini?')) { e.remove(); urutkanNomor(); } }
        function urutkanNomor() { const d = Array.from(document.getElementById('tbody-' + currentTab).children).filter(tr => !tr.querySelector('td[colspan]') && tr.id !== 'tr-tambah-baris'); d.forEach((tr, i) => tr.children[0].innerText = i + 1); }
        ...
        function updateButtonVisual(is) { const b = document.getElementById('btn-edit-data'), l = currentTab.toUpperCase(); if(is) { b.innerHTML = `<svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg> Simpan Data ${l}`; b.className = "flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md group"; } else { b.innerHTML = `<svg class="w-4 h-4 text-gray-300 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg> Edit Data ${l}`; b.className = "flex items-center gap-2 bg-gray-800 hover:bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md group"; } }

        // ===== LOGIKA TAMBAH & HAPUS MINGGU =====
        let totalMinggu = 4;
        const MAKSIMAL_MINGGU = 5;

        function tambahMinggu() {
            if (totalMinggu >= MAKSIMAL_MINGGU) return; 
            ...
            // Kosongkan teks data, ABAIKAN kolom No(0), Waktu(7), Dok(13), dan Aksi(14)
            let cells = tbodyBaru.querySelectorAll('td');
            cells.forEach((cell, index) => {
                let colIndex = index % 16; // Sekarang ada 16 kolom di PEMT
                if (![0, 8, 14, 15].includes(colIndex)) {
                    if(cell.innerText.includes('%')) cell.innerText = '0.00%';
                    else if(cell.innerText === '0' || !isNaN(parseFloat(cell.innerText.replace(/,/g, '')))) cell.innerText = '0';
                    else cell.innerText = '-';
                }
            });
